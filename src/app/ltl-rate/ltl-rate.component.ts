...
@Component({
    selector: 'app-ltl-rate',
    templateUrl: './ltl-rate.component.html',
    styleUrls: ['./ltl-rate.component.css']
})
export class LtlRateComponent implements OnInit {
    ...
    public retrievingType = 'previousWeekData';
    ...
    search(myTracking:any) {
        if (myTracking.salesRepId === '' && myTracking.bolTrackingNumber === '' && myTracking.companyId === '' && myTracking.bolCarrier === '' && myTracking.shipperZip === '' &&
            myTracking.consigneeZip === '' && myTracking.quoteNumber === '') {
            this.showErrorMessage = false;
    ...
    getBillofLadingDetails() {
        ...
        this.showErrorMessage = false;
        let viewType = 'ltl';
        this.externalService.getTrackDetailsOfBOL(this.externalCustomerParseData.companyId, this.externalCustomerParseData.salesRepId, this.accessToken, this.loginType, dateNew, viewType, this.retrievingType).subscribe((data:any) => {
            ...
            if (this.trackingDetails.length > 0) {
                ...
                for (let t = 0; t < this.trackingDetails.length; t++) {
                    ...
                    if (this.trackingDetails[t].bolCarrier === 'FEDEX_FREIGHT_PRIORITY') {
                    ...
                    else if (this.trackingDetails[t].bolCarrier === 'REDDAWAY') {
                        this.trackingDetails[t].bolResponse = this.trackingDetails[t].bolResponse;
                        this.trackingDetails[t].trackingNumber = this.trackingDetails[t].bolResponse;
                        ...
                        this.trackingDetails[t].bolResponse = this.trackingDetails[t].bolResponse;
                        this.trackingDetails[t].trackingNumber = this.trackingDetails[t].bolResponse;
                        console.log('1 Others', this.trackingDetails[t].bolResponse);
                    }
                    console.log('response', this.trackingDetails[t].bolResponse);
                }
            } else {
        ...
        this.externalService.getMyBolType(this.accessToken, bolReferenceNumber).subscribe((data:any) => {
            ...
            if (this.bolValue.bolCarrier === 'YRC') {
                ...
                if (this.bolValue.bolRequest.billPaidTo.name === "BILLTOFORTE") {
                    ...
                    if(this.bolValue.bolRequest.billPaidTo.name === 'PREPAID' || this.bolValue.bolRequest.billPaidTo.name === 'PREPAY') {
    ...
    updateTrackProNumber(value:any) {
        ...
        this.pricingInfoService.updateBolTrackNumber(value).subscribe((response:any) => {
            ...
            if (this.updateResponse === true) {
                Swal.fire({
                    title: 'Updated!',
                    text: 'Updated Pro/Track Number Sucessfully!',
                    icon: 'success',
                });
            } else {
                Swal.fire({
                    ...
                    text: 'Failed to update Pro/Track Number!',
                    icon: 'error'
                });
            }
            this.localStorageValues();
    ...
    retrieveAllData(type:any) {
        ...
        if (type === 'showAll') {
        this.retrievingType = 'allData';
        this.getBillofLadingDetails();    
    } else {
        this.retrievingType = 'previousWeekData';
        this.getBillofLadingDetails();
    }
