...
import { ApplicationComp } from '../util/application-comp';
...
export class InternalSalesComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private pricingInfoService: PricingInfoService,
    ...
    this.applicationComp = new ApplicationComp(customerService, pricingInfoService, quoteReportService, loggerService);
  ...
  ngOnInit() {
    ...
    this.information();
    this.applicationComp.emergencyStatus();
    this.customerId = 0;
  ...
  */
  getSalesData() {
    this.getApData();
    this.applicationComp.getArData();
  }
  ...
  getApData() {
    this.pricingInfoService.getApForm().subscribe(data => {
      ...
      this.applicationComp.logger = {
        'method': 'getApForm',
        'message': 'AP(discount,minimum charge, fuel charge)',
        'AP Data': 'Ap Full Data'
      };
      this.loggerService.info(this.applicationComp.logger);
    });
  }
  ...
  getAllCompanyNotes() {
    this.customerService.getCompanyDetails(this.accessToken).subscribe(getCustomerData => {
      ...
      console.log('this.customerFeatures', this.customerFeaturesAll);
      this.applicationComp.logger = { 'method': 'getCompanyDetails', 'message': 'Customers', 'CustomerDetail ': 'All' };
      this.loggerService.info(this.applicationComp.logger);
    });
  }
  ...
  checkForZipCode(zipCode: any, type: any) {
    ...
    if (zipCode.target.value.length === 5) {
      ...
      if (zipCode.target.value === '') {
        ...
        this.applicationComp.errorMessage = false;
        this.showRemoveIconDest = false;
        this.applicationComp.errorMessageDest = false;
        this.applicationComp.showZipcodes = false;
        this.applicationComp.showZipcodeDest = false;
      } else {
        if (type === 'origin') {
          this.applicationComp.noshipmentData = false;
      ...
      if (this.getZipcodeValues.length > 0) {
        for (let i = 0; i < this.getZipcodeValues.length; i++) {
          if (type === 'origin') {
            ...
            this.applicationComp.filterValues = this.getZipcodeValues[i];
            this.applicationComp.filterValuesState = this.getZipcodeValues[i].state;
            if (this.applicationComp.filterValuesState === 'HI' || this.applicationComp.filterValuesState === 'AK') {
              this.applicationComp.noshipmentData = true;
            } else {
              this.applicationComp.showZipcodes = true;
              this.applicationComp.errorMessage = false;
              (document.getElementById('destinationZipCode') as HTMLFormElement).focus();
            }
          } else {
            this.applicationComp.filterValuesDest = this.getZipcodeValues[i];
            this.applicationComp.filterValuesDestState = this.getZipcodeValues[i].state;
            if (this.applicationComp.filterValuesDestState === 'HI' || this.applicationComp.filterValuesDestState === 'AK') {
              this.noshipmentDestData = true;
            } else {
              this.applicationComp.showZipcodeDest = true;
              this.applicationComp.errorMessageDest = false;
              (document.getElementById('classification') as HTMLFormElement).focus();
            }
          }
        ...
        if (type === 'origin') {
          this.applicationComp.showZipcodes = false;
          this.applicationComp.errorMessage = true;
          (document.getElementById('originZipCode') as HTMLFormElement).focus();
        } else {
          this.applicationComp.showZipcodeDest = false;
          this.applicationComp.errorMessageDest = true;
          (document.getElementById('destinationZipCode') as HTMLFormElement).focus();
        }
      }
      this.applicationComp.logger = { 'method': 'getCityState', 'message': 'Info Message', 'Zipcode': zipCode };
      this.loggerService.info(this.applicationComp.logger);
    });
  }
  ...
  getCityState1(zipCode: any, type: any) {
    this.getQuoteFlag = true;
    this.applicationComp.showTableZip = false;
    this.applicationComp.showTableZip1 = false;

    this.pricingInfoService.getCityState(zipCode).subscribe((getArrayValues: any) => {
      console.log(getArrayValues);
      this.applicationComp.originZipArray = [];
      ...
      if (cityAndState && cityAndState.length > 0) {
        ...
        if (CityArray.length > 1) {
          if (type === 'origin') {
            this.applicationComp.showTableZip = true;
            CityArray.forEach((ele: any) => {
              this.applicationComp.originZipArray.push({ 'cityName': ele, 'state': cityAndState[0].state, 'selectedFocus': false })
            })
            // $('#0').focus();
            this.applicationComp.originZipArray[0].selectedFocus = true;
            setTimeout(() => {
            ...
            this.applicationComp.selectedIndex = 0;
          } else {
            this.applicationComp.showTableZip1 = true;
            CityArray.forEach((ele: any) => {
          ...
          if (type === 'origin') {
            this.applicationComp.filterValues = cityAndState[0].city;
            this.applicationComp.filterValuesState = cityAndState[0].state;
            if (this.applicationComp.filterValuesState === 'HI' || this.applicationComp.filterValuesState === 'AK') {
              this.applicationComp.noshipmentData = true;
            } else {
              this.applicationComp.showZipcodes = true;
            ...
            console.log(this.applicationComp.showZipcodes, this.applicationComp.filterValues, this.applicationComp.filterValuesState);
          } else {
            let obbb = { city: cityAndState[0].city, state: cityAndState[0].state }

            this.applicationComp.filterValuesDest = obbb;
            this.applicationComp.filterValuesDestState = cityAndState[0].state.$value;
            if (this.applicationComp.filterValuesDestState === 'HI' || this.applicationComp.filterValuesDestState === 'AK') {
              this.noshipmentDestData = true;
            } else {
              this.applicationComp.showZipcodeDest = true;
              this.applicationComp.errorMessageDest = false;
              this.applicationComp.showErrorMessageForClass = false;
              this.applicationComp.showChooseClass = false;
              (document.getElementById('classification') as HTMLFormElement).focus();
            }
          }
  ...
  checkKeyInput(event: any) {
    console.log(event);
    if (event === 13) {
      console.log(this.applicationComp.selectedIndex);
      // $('#buttoniD').focus();
      $('tabindex[' + this.applicationComp.selectedIndex + ']').focus();
    }
  }
  ...
  checkForClass(event: any) {
    ...
    if (event.target.value !== '') {
      ...
      if (classificationData.length > 0) {
        ...
        this.applicationComp.showErrorMessageForClass = true;
        (document.getElementById('classification') as HTMLFormElement).focus();
        setTimeout(() => {
          this.applicationComp.showErrorMessageForClass = false;
        }, 5000);
  ...
  clear() {
    ...
    this.applicationComp.filterValues = '';
    this.applicationComp.filterValuesDest = '';
    this.pricingDetail = [];
    this.showRemoveIcon = false;
    this.showRemoveIconDest = false;
    this.selectedItems = [];
    ...
    (document.getElementById('originZipCode') as HTMLFormElement).focus();
    this.applicationComp.logger = { 'method': 'getCityState', 'message': 'Clear all values' };
    this.loggerService.debug(this.applicationComp.logger);
  }
 ...
 */
  ...
  changeArValues(profile: any) {
    ...
    if (profile !== undefined) {
      if (profile.type === 'AR') {
        for (let i = 0; i < this.applicationComp.listOfArValues.length; i++) {
          if (profile.companyName === this.applicationComp.listOfArValues[i].companyName) {
            this.applicationComp.listOfArValues.splice(i, j);
            j++;
          }
        }
        this.applicationComp.listOfArValues.push(profile);
        let sortOrder;
        sortOrder = this.applicationComp.listOfArValues.filter(function (el: any) {
          return el.companyName === 'FEDEX PRIORITY';
        });
        let sortOrder1;
        sortOrder1 = this.applicationComp.listOfArValues.filter(function (el: any) {
          return el.companyName === 'FEDEX ECONOMY';
        });
        let sortOrder2;
        sortOrder2 = this.applicationComp.listOfArValues.filter(function (el: any) {
          return el.companyName === 'YRC';
        });
        let sortOrder3;
        sortOrder3 = this.applicationComp.listOfArValues.filter(function (el: any) {
        ...
        this.applicationComp.listOfArValues = arraySorting;
      } else {
  ...
  giveRateDetails(rateDetail: any) {
    ...
    if (rateDetail.length > 1) {
      ...
      for (let r = 0; r < rateDetail.length; r++) {
        ...
        this.quoteDetails = {
          ...
          originCityState: JSON.stringify({ 'city': this.applicationComp.filterValues, 'state': this.applicationComp.filterValuesState }),
          destinationCityState: JSON.stringify({ 'city': this.applicationComp.filterValuesDest.city, 'state': this.applicationComp.filterValuesDest.state })
        };
    ...
    if (this.sendArrayValues.length > 1) {
      this.pricingInfoService.saveQuoteDetails(this.sendArrayValues).subscribe((data: any) => {
        let date = new Date();
        console.log('QUOTE RESPONSE', data, date);
        ...
        this.applicationComp.logger = { 'method': 'saveQuoteDetails', 'message': 'Quote Number is generated', 'Quote Number': data };
        this.loggerService.info(this.applicationComp.logger);
      });
    }
  }
  ...
  netChargeArrSum(netCharge: any) {
    ...
    netCharge.forEach(function (key: any) {
      ...
      console.log("Total easwari", total);
    });
    return total;
  }
  ...
  getQuote(transportOrigin: any, selectedItems: any, pricingDetail: any) {
    ...
    classAndWeight = this.applicationComp.checkForClassification(classWeight);
    ...
    this.userData.originState = this.applicationComp.filterValuesState;
    this.userData.originCity = this.applicationComp.filterValues;
    this.userData.destinationState = this.applicationComp.filterValuesDest.state;
    this.userData.destinationCity = this.applicationComp.filterValuesDest.city;
    ...
    this.applicationComp.logger = { 'method': 'getQuote', 'message': 'Passing Request to the Api methods', 'request': this.userData, 'timeStamp': date };
    this.loggerService.info(this.applicationComp.logger);
    this.pricingInfoService.getRates(this.userData, 'YRC AP').subscribe((response: any) => {
      console.log('RESPONSE YRC AP', response);
      this.getRatesYRCAPResponse(response, 'YRC AP');
    },
      (err: any) => {
        this.applicationComp.logger = { 'method': 'getRates', 'message': 'YRC AP Error', 'status': err.status };
        this.loggerService.error(this.applicationComp.logger);
        ...
        if (err.status === 0) {
          ...
          setTimeout(() => {
            ...
            this.pricingInfoService.getRates(this.userData, 'YRC AP').subscribe((response: any) => {
              ...
              this.getRatesYRCAPErrResponse(err);            
            });
    ...
    if (this.applyCostPlusForYRC === false) {
      this.pricingInfoService.getRates(this.userData, 'YRC AR').subscribe((response: any) => {
       ...
       this.getRatesYRCARResponse(response);
      }, (err: any) => {
        this.applicationComp.logger = { 'method': 'getRates', 'message': 'YRC AR Error response', 'status': err.status };
        this.loggerService.error(this.applicationComp.logger);
        // document.getElementById('myDiv').className = 'imgBlock';
        if (err.status === 0) {
          // this.showErrorInternetConnection = true;
    ...
    setTimeout(() => {
      this.pricingInfoService.getRates(this.userData, 'YRC OLDAP').subscribe((response: any) => {
        console.log('RESPONSE YRC AP', response);
        this.getRatesYRCAPResponse(response, 'YRC OLDAP');
      }, (err: any) => {
        this.applicationComp.logger = { 'method': 'getRates', 'message': 'YRC AP Error', 'status': err.status };
        this.loggerService.error(this.applicationComp.logger);
        // document.getElementById('myDiv').className = 'imgBlock';
    ...
    this.pricingInfoService.getRates(this.userData, 'FEDEX AP').subscribe((response:any) => {
     ...
     this.getRatesFEDEXAPResponse(response,'true');
     (err:any) => {
      this.applicationComp.logger = { 'method': 'getRates', 'message': 'Fedex AP error response', 'status': err.status };
      this.loggerService.error(this.applicationComp.logger);
      // document.getElementById('myDiv').className = 'imgBlock';
      if (err.status === 0) {
        // this.showErrorInternetConnection = true;
        ...
        setTimeout(() => {
          this.pricingInfoService.getRates(this.userData, 'FEDEX AP').subscribe((response:any) => {
            console.log('RESPONSE FEDEX AP', response);
            this.getRatesFEDEXAPResponse(response,'false');
          
          }, (err:any) => {
          this.getRatesFEDEXErrResponse(err);
          });
    ...
    if (this.applyCostPlusForFedexEco === false || this.applyCostPlusForFedexPri === false) {
      this.pricingInfoService.getRates(this.userData, 'FEDEX AR').subscribe((response: any) => {
       ...
       this.getRatesFEDEXARResponse(response);
      }, (err: any) => {
        this.applicationComp.logger = { 'method': 'getRates', 'message': 'Fedex AR error response', 'status': err.status };
        this.loggerService.error(this.applicationComp.logger);
        // document.getElementById('myDiv').className = 'imgBlock';
        if (err.status === 0) {
          // this.showErrorInternetConnection = true;
    ...
    this.getRatesnewREDDAWAYAPResponse(response, 'REDDAWAY AP');
    }, (err: any) => {
    this.getRatesnewREDDAWAYErrResponse(err);
    });
    this.pricingInfoService.getRates(this.userData, 'REDDAWAY OLDAP').subscribe((response: any) => {
      console.log('RESPONSE REDDAWAY AP', response);
      this.getRatesnewREDDAWAYAPResponse(response,'REDDAWAY OLDAP');//do
    }, (err: any) => {
      this.applicationComp.logger = { 'method': 'getRates', 'message': 'REDDAWAY AP Error', 'status': err.status };
      this.loggerService.error(this.applicationComp.logger);
      // document.getElementById('myDiv').className = 'imgBlock';
      if (err.status === 0) {
        // this.showErrorInternetConnection = true;
        ...
        setTimeout(() => {
          this.pricingInfoService.getRates(this.userData, 'REDDAWAY OLDAP').subscribe((response: any) => {
            ...
            this.applicationComp.logger = { 'method': 'getRates', 'message': 'REDDAWAY AP', 'response': 'Pass', 'timeStamp': reddawayApTimestamp };
            this.loggerService.info(this.applicationComp.logger);
              ...
              if (Object.keys(response.result).length === 0 || response.result === undefined) {
                ...
                if (response.result === 'service not available') {
                  ...
                  if (this.applyCostPlusForReddaway === true) {
                    ...
                    this.getTransitTimeReddaway(object);
            ...
            this.getRatesnewREDDAWAYAPERRResponse(err);
    ...
    if (this.applyCostPlusForReddaway === false) {
      this.pricingInfoService.getRates(this.userData, 'REDDAWAY AR').subscribe((response: any) => {
        ...
        this.getRatesREDDAWAYARResponse(response);
      }, (err: any) => {
        this.applicationComp.logger = { 'method': 'getRates', 'message': 'REDDAWAY AR Error response', 'status': err.status };
        this.loggerService.error(this.applicationComp.logger);
        // document.getElementById('myDiv').className = 'imgBlock';
        if (err.status === 0) {
          // this.showErrorInternetConnection = true;
  ...
  getRatesnewREDDAWAYAPERRResponse(err: any){
            this.applicationComp.logger = { 'method': 'getRates', 'message': 'REDDAWAY AP Error', 'status': err.status };
            this.loggerService.error(this.applicationComp.logger);
            (document.getElementById('myDiv') as HTMLFormElement).className = 'imgBlock';
            if (err.status === 0) {
  ...
  getRatesYRCAPErrResponse(err: any){
  this.applicationComp.logger = { 'method': 'getRates', 'message': 'YRC AP Error', 'status': err.status };
  this.loggerService.error(this.applicationComp.logger);
  (document.getElementById('myDiv') as HTMLFormElement).className = 'imgBlock';
  if (err.status === 0) {
  ...
  getRatesnewREDDAWAYErrResponse(err: any){
    this.applicationComp.logger = { 'method': 'getRates', 'message': 'REDDAWAY AP Error', 'status': err.status };
    this.loggerService.error(this.applicationComp.logger);
    // document.getElementById('myDiv').className = 'imgBlock';
    if (err.status === 0) {
  ...
  getRatesnewREDDAWAYAPResponse(response: any, data: any){ 
    let reddawayApTimestamp = new Date();
    console.log('RESPONSE TIME For REDDAWAY AP', reddawayApTimestamp);
    this.applicationComp.logger = { 'method': 'getRates', 'message': 'REDDAWAY AP', 'response': 'Pass', 'timeStamp': reddawayApTimestamp };
    this.loggerService.info(this.applicationComp.logger);
    if (response.result === 'Shipments cannot be done to Alaska and Hawaii state. Please contact customer care.') {
      ...
      if (Object.keys(response.result).length === 0 || response.result === undefined) {
        ...
        if (response.result === 'service not available') {
         ...
         data === 'REDDAWAY AP'? this.rateCalculationForYRCReddaway1(response, 'REDDAWAY', 'AP', 'AP')
         :  this.rateCalculationForYRCReddaway2(response, 'REDDAWAY', 'AP', 'AP');; //differ
          if (this.applyCostPlusForReddaway === true) {
            let object = this.userData;
            object.carrier = "REDDAWAY";
            this.pricingInfoService.getSingleShipment(object).subscribe((data: any) => {
              this.getRuleForReddawayAR = data.result;
              data === 'REDDAWAY AP'? this.rateCalculationForYRCReddaway1(response, 'REDDAWAY', 'AP', 'COSTPLUS'):
              this.rateCalculationForYRCReddawayCostplus(response, 'REDDAWAY', 'AP', 'COSTPLUS'); // differ
            }, (err: any) => {
              this.getRuleForReddawayAR = [];
              data === 'REDDAWAY AP'? this.rateCalculationForYRCReddaway1(response, 'REDDAWAY', 'AP', 'COSTPLUS')
              :this.rateCalculationForYRCReddawayCostplus(response, 'REDDAWAY', 'AP', 'COSTPLUS'); //differ
    ...
    getTransitTimeFEDEXPriority(object: any){
    this.pricingInfoService.getTransitTime(object).subscribe((response:any) => {
      ...
      if (this.transitTimeFedexPriority.result !== false) {
        ...
        this.applicationComp.numbersArray.forEach((elem:any) =>{
    ...
    getTransitTimeFEDEXECONOMY(objectForEco: any){
    this.pricingInfoService.getTransitTime(objectForEco).subscribe((response: any) => {
      console.log('transitTime', objectForEco);
    ...
    getTransitTimeReddaway(object: any){
      this.pricingInfoService.getTransitTime(object).subscribe((response: any) => {
        console.log('transitTime', object);
  ...
  getRatesFEDEXARResponse(response: any){
    let fedexArTimestamp = new Date();
    console.log('RESPONSE TIME FOR FEDEX AR', fedexArTimestamp);
    this.applicationComp.logger = { 'method': 'getRates', 'message': 'FEDEX AP', 'response': 'Pass', 'timeStamp': fedexArTimestamp };
    this.loggerService.info(this.applicationComp.logger);
    if (response.result === 'Shipments cannot be done to Alaska and Hawaii state. Please contact customer care.') {
      (document.getElementById('myDiv') as HTMLFormElement).className = 'imgBlock';
      this.showAllData = true;
      ...
      if (Object.keys(response.result).length === 0 || response.result === undefined) {
        ...
        this.getTransitTimeFEDEXECONOMY(objectForEco);
        

        let object = this.userData;
        object.carrier = "FEDEX PRIORITY"
        this.getTransitTimeFEDEXPriority(object);
  ...
  getRatesREDDAWAYARResponse(response: any){
    let reddawayArTimestamp = new Date();
    console.log('RESPONSE TIME FOR REDDAWAY AR', reddawayArTimestamp);
    this.applicationComp.logger = { 'method': 'getRates', 'message': 'REDDAWAY AR', 'response': 'Pass', 'timeStamp': reddawayArTimestamp };
    this.loggerService.info(this.applicationComp.logger);
    if (response.result === 'Shipments cannot be done to Alaska and Hawaii state. Please contact customer care.') {
      (document.getElementById('myDiv') as HTMLFormElement).className = 'imgBlock';
  ...
  getRatesFEDEXErrResponse(err: any){
    this.applicationComp.logger = { 'method': 'getRates', 'message': 'Fedex AP error response', 'status': err.status };
    this.loggerService.error(this.applicationComp.logger);
    (document.getElementById('myDiv')as HTMLFormElement).className = 'imgBlock';
    if (err.status === 0) {
 ...
 getRatesFEDEXAPResponse(response: any, noerror: any){
  let fedexApTimestamp = new Date();
  console.log('RESPONSE TIME FOR FEDEX AP', fedexApTimestamp);
  this.applicationComp.logger = { 'method': 'getRates', 'message': 'FEDEX AP', 'response': 'Pass', 'timeStamp': fedexApTimestamp };
  this.loggerService.info(this.applicationComp.logger);
  if (response.result === 'Shipments cannot be done to Alaska and Hawaii state. Please contact customer care.') {
    (document.getElementById('myDiv')as HTMLFormElement).className = 'imgBlock';
    this.showAllData = true;
    // this.hideFedexEcoData = true;
  noerror === 'true'?  this.nofedexecoData = true :  this.nofedexecoData = false;   
    // this.hideFedexPriData = true;
    this.showFedexPriData = false;
    this.showFedexEcoData = false;
    this.nofedexpriData = true;
  ...
  if (Object.keys(response.result).length === 0 || response.result === undefined) {
    ...
    if (this.applyCostPlusForFedexEco === true) {
      ...
      this.getTransitTimeFEDEXECONOMY(object);
  ...
  getRatesYRCAPResponse(response: any, rateCalculationForYRCReddaway2: any) {
    let yrcApTimestamp = new Date();
    console.log('RESPONSE TIME FOR YRC AP', yrcApTimestamp);
    this.applicationComp.logger = { 'method': 'getRates', 'message': 'YRC AP', 'response': 'Pass', 'timeStamp': yrcApTimestamp };
    this.loggerService.info(this.applicationComp.logger);
    if (response.result === 'Shipments cannot be done to Alaska and Hawaii state. Please contact customer care.') {
      (document.getElementById('myDiv') as HTMLFormElement).className = 'imgBlock';
      ...
      if (Object.keys(response.result).length === 0 || response.result === undefined) {
        ...
        rateCalculationForYRCReddaway2 === 'YRC OLDAP' ? this.rateCalculationForYRCReddaway2(response, 'YRC', 'AP', 'AP'): this.rateCalculationForYRCReddaway1(response, 'YRC', 'AP', 'AP');
        if (this.applyCostPlusForYRC === true) {
          ...
          this.pricingInfoService.getSingleShipment(object).subscribe((data: any) => {
            ...
            rateCalculationForYRCReddaway2 === 'YRC OLDAP' ? this.rateCalculationForYRCReddaway1(response, 'YRC', 'AP', 'COSTPLUS'): this.rateCalculationForYRCReddawayCostplus(response, 'YRC', 'AP', 'COSTPLUS');
          }, (err: any) => {
  ...
  getRatesYRCARResponse(response: any) {
    let yrcArTimestamp = new Date();
    console.log('RESPONSE TIME FOR YRC AR', yrcArTimestamp);
    this.applicationComp.logger = { 'method': 'getRates', 'message': 'YRC AR', 'response': 'Pass', 'timeStamp': yrcArTimestamp };
    this.loggerService.info(this.applicationComp.logger);
    if (response.result === 'Shipments cannot be done to Alaska and Hawaii state. Please contact customer care.') {
      (document.getElementById('myDiv') as HTMLFormElement).className = 'imgBlock';
  ...
  rateCalculationForYRCReddaway(response: any, carrierType: any, category: any, factoring: any) {
    ...
    if (factoring !== 'COSTPLUS') {
      for (let i = 0; i < this.parseSetMasterData.length; i++) {
        if (this.parseSetMasterData[i].companyName === carrierType) {
          ...
          if (this.applicationComp.filterValues.state === 'CA' || this.applicationComp.filterValuesDest.state === 'CA') {
    ...
    if (carrierType === 'REDDAWAY') { //differ
  ...
  rateCalculationForFedex(response: any, carrierType: any, category: any, factoring: any) {
    ...
    if (factoring !== 'COSTPLUS') {
      for (let i = 0; i < this.parseSetMasterData.length; i++) {
        if (this.parseSetMasterData[i].companyName === carrierType) {
          ...
          this.changeArValues(this.profile);
          console.log('Filtevalues', this.applicationComp.filterValues);
          if (this.applicationComp.filterValues.state === 'CA' || this.applicationComp.filterValuesDest.state === 'CA') {
            if (this.parseSetMasterData[i].caCharge !== '' && this.parseSetMasterData[i].caCharge !== null && this.parseSetMasterData[i].caCharge !== undefined) {
              addCACharge = Number(this.parseSetMasterData[i].caCharge);
            } else {
      ...
      for (let i = 0; i < this.localStorageArData.length; i++) {
        if (this.localStorageArData[i].companyName === carrierType) {
          ...
          console.log('Filtevalues1', this.applicationComp.filterValues);

          if (this.applicationComp.filterValues.state === 'CA' || this.applicationComp.filterValuesDest.state === 'CA') {
            console.log('Filtevalues2', this.applicationComp.filterValues);
    ...
    netChargeArray.push(netCharge.toFixed(2));
    console.log('netChargeArray', netChargeArray, netCharge, newDiscountedRate, fuelSurcharge, highCost);
    ...
    //  const netChargeResult = ((discountedRate) * (fuelSurcharge / 100));
    netChargeResultArray.push(netChargeResult.toFixed(2));
    const netChargeValue = this.netChargeArrSum(netChargeArray); //TEST HERE
    let assessorialUnique = [], uniqueArray = [];
    console.log('this.selectedItems before duplicating', this.selectedItems);
    //  assessorialUnique = this.getUnique(this.selectedItems, 'name');
    //  console.log('assessorialUnique', assessorialUnique);
    ...
    console.log(totalAssessorialCharge, totalCharge, Number(netChargeValue), Number(additionalRate), Number(addSingleShipmentCharge), Number(assessorialCharge), Number(addCACharge), Number(peakCharge));
    totalChargeArray.push(totalCharge.toFixed(2));
    classification1.splice(0, 2);
    const totalChargeForComparing = this.netChargeArrSum(totalChargeArray);
